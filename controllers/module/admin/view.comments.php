...
$query = "SELECT seq, seq_root, stamp, content_stripped as content, deleted FROM " . COMMENT_DATA_TABLE . " WHERE seq_member='".$in['seq_member']."'";

// 사용자 정보 
$info = $sy['mb']->info($in['seq_member']);

$no_of_post = 40;
$total_post = $sy['db']->count(COMMENT_DATA_TABLE, array('seq_member'=>$in['seq_member']));

$page_no = $sy['post']->page_no($in['page_no']);

$posts = $sy['post']->posts ( $query, $page_no, $no_of_post );
?>
<form id='view-comment-form' method='post' target='hiframe'>
	<input type='hidden' name='module' value='admin' />
	<input type='hidden' name='action' value='view_comment_submit' />
	...
	foreach ( $posts as $p ) {
		...
		$subject = stringcut($p['content'], 120);
