...
?>
<div id='admin-member-edit'>
	<fieldset>
			<legend>회원정보</legend>
			<div class='admin-row'><span class='sub-title'>번호</span><?=number_format($info['seq'])?></div>
			<div class='admin-row'><span class='sub-title'>가입일</span><?=date('Y-m-d H:i:s', $info['reg_stamp'])?></div>
			<div class='admin-row'><span class='sub-title'>가입IP</span><?=long_2_ip($info['ip'])?></div>
			<div class='admin-row'><span class='sub-title'>아이디</span><?=$info['username']?></div>
	<?php		
		if ( $info['block_stamp'] ) {  // 차단된 회원인 경우?>	
			<div>
				...
				<form method='post' target='hiframe'>
					...
					<input type='hidden' name='action' value='admin.member.edit_submit' />
	...
	<? }?>
		<form method='post' target='hiframe' autocomplete='off'>
			...
			<input type='hidden' name='seq' value='<?=$info['seq']?>' />
				...
				<div class='admin-row'>
					<input type='checkbox' name='is_admin' value='Y' <?=$info['is_admin']?"checked":""?> />최고 관리자
				</div>
