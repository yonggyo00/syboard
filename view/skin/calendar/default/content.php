...
for( $i=0; $i < 42; $i++ ) {
	...
	if ( $start_day > $i ) echo "<div class='cel'></div>";
			...
			if ( $date > $last_date ) echo "<div class='cel $bottom_line'></div>";
				...
				echo "<div class='cel cel_view_schedule $bottom_line $today $add_class' popup_url='?module=calendar&action=view_schedule&layout=1&date=".$skin_year."-".$skin_month."-".$date."'>$date</div>";
			}
		}
	if ( $i % 7 == 6 ) {
		echo "
