...
// 어드민 또는 게시판 어드민인 경우 공지를 나타낸다.
	...
	?>
		<input type='checkbox' name='reminder' value=1 <?=$p['reminder']?'checked':''?> /><?=lang('Write_form reminder')?>
	<?}?>
<?php
	...
	if ( function_exists('after_write_subject') ) {
		...
		if ( empty($post_cfg['use_login_post']) && !$sy['mb']->is_login() ) {
			if ( $in['action'] == 'write' && !$sy['mb']->is_login() ) {?>
				<?=lang('Write_form poster')?><input type='text' name='guest_username' placeholder='<?=lang('Write_form poster')?>' />
				<?=lang('Write_form post_password')?><input type='password' name='guest_secret' placeholder='<?=lang('Write_form secret password')?>' />
			<?}
			if ( $site_config['use_captcha'] ) {
				echo "<div>".lang('Write_form captcha');
...
<?php
	if ( $post_cfg['blog_api_use'] ) {
		 for ( $i=1; $i<=3; $i++ ) {?>
			<div>
				<?=lang('Write_form blog_account')?><?=$i?> <input type='text' name='blog_category_<?=$i?>' value='<?=$p['blog_category_'.$i]?>' placeholder='<?=lang('Write_form blog_category')?><?=$i?>' />
